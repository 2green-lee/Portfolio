...
// Get imports from topPart robustly
// We will extract all lines starting with 'import ' until a non-import line that doesn't start with space/brace (rudimentary, or just grab the first N lines that look like imports)
const importLines = [];
const lines = topPart.split('\n');
for(let line of lines) {
  if(line.startsWith('import ') || line.startsWith('} from ') || line.startsWith('  ')) {
    // Collect potential import block. A safer way is regex.
  }
}
const importSectionMatch = topPart.match(/^(?:import\s+[\s\S]*?from\s+['"][^'"]+['"];?\s*)+/m);
